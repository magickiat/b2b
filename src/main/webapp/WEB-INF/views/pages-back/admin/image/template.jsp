...
<body>
	...
	<div class="container">
		<div class="row">
			<div class="col-sm-12 bg_color showline2">
				<div class="row col-md-6 row-header2 header2 txtupper">Template Image upload</div>
		...
		<div class="row bg_color showline2" style="margin-top: 10px;">
			<form action='<c:url value="/backend/admin/template/list"></c:url>'>
				...
				<div class="col-sm-4" style="">
					...
					&nbsp;
					<input type="button" class="btn btn-default" value="Reset" onclick="gotoImagePage();" />
				</div>
				<div class="col-sm-12">&nbsp;</div>
			</form>
		</div>
		...
		<c:set var="baseUrl" value="/backend/admin/template/list?keyword=${ keyword }&folder=${ folder }" />
						...
						<tr>
							...
							<td>
								<c:choose>
									...
									<c:otherwise>
										<a href='<c:url value="/upload/${ item.nameWithPath }" />'>
											<img style="width: 80px;" src="<c:url value="/upload/${ item.nameWithPath }" />" /> ${ item.name }
										</a>
									</c:otherwise>
								</c:choose>
							</td>
	...
	<script type="text/javascript">
				...
				.on(
						...
						function() {
							...
							if (files.length > 0) {
								...
								$
										.post(
												'<c:url value="/backend/admin/template/delete.json" />',
												param)
										.done(
		...
		function list(path) {
			...
			window.location.href = '<c:url value="/backend/admin/template/list?currentPath='
					+ pathValue
					+ '&folder='
					+ $('#selectedFolder').val()
					+ '" />';
		...
		var uploadURL = '<c:url value="/backend/admin/template/upload.json" />';
		...
		function gotoImagePage(){
			window.location.href='<c:url value="/backend/admin/template/list" />';
		}
	</script>
</body>
</html>
