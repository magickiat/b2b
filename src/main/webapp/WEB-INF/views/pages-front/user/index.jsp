											...
											<div>
												<table>
													<tr>
														...
														<td rowspan="4" valign="top"><textarea id="address${row.index}" name="addresses[${row.index}].address" rows="4"
																cols="50">${ address.address }</textarea></td>
														<td>Country :</td>
														<td><select id="country${row.index}"
														...
														<td>Telephone :</td>
														<td><form:input id="tel1${row.index}" path="addresses[${row.index}].tel1"
																class="form-control" value="${ address.tel1 }" /></td>
													</tr>
													<tr>
														<td>PostCode :</td>
														<td><form:input id="postcode${row.index}" path="addresses[${row.index}].postCode"
																class="form-control" value="${ address.postCode }" /></td>
														<td>Fax :</td>
														<td><form:input id="fax${row.index}" path="addresses[${row.index}].fax" class="form-control"
																value="${ address.fax }" /></td>
													</tr>
													<tr>
														<td>Email :</td>
														<td><form:input id="email${row.index}" path="addresses[${row.index}].email"
																class="form-control" value="${ address.email }" /></td>
														<td>Address Type :</td>
														<td><select id="addressType${row.index}" name="addresses[${row.index}].type">
																<option value="" disabled="disabled">---please select---</option>
																<c:forEach items="${ addressTypes }" var="addressType">
																	<option value="${ addressType }"
																		${addressType==address.type?'selected="selected"':''}>${ addressType }</option>
																</c:forEach>
														</select>
													</tr>
													<tr>
