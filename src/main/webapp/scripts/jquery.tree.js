...
(function($) {
	...
	$.tree = {
		...
		defaults : {
			data : {
				async : false, // Are async requests used to load open_branch
								// contents
			...
			languages : [], // ARRAY of string values (which will be used as CSS
							// classes - so they must be valid)
			ui : {
				dots : true, // BOOL - dots or no dots
				animation : 0, // INT - duration of open/close animations in
								// miliseconds
				scroll_spd : 4,
				theme_path : false, // Path to the theme CSS file - if set to
									// false and theme_name is not false - will
									// lookup
									// jstree-path-here/themes/theme-name-here/style.css
				theme_name : "default",// if set to false no theme will be
										// loaded
			...
			types : {
				"default" : {
					...
					max_children : -1, // -1 - not set, 0 - no children, 1 -
										// one child, etc // can be function
					max_depth : -1, // -1 - not set, 0 - no children, 1 - one
									// level of children, etc // can be function
			...
			rules : {
				multiple : false, // FALSE | CTRL | ON - multiple selection
									// off/ with or without holding Ctrl
				multitree : "none", // all, none, array of tree IDs to accept
									// from
				type_attr : "rel", // STRING attribute name (where is the type
									// stored as string)
				createat : "bottom", // STRING (top or bottom) new nodes get
										// inserted at top or bottom
				drag_copy : "ctrl", // FALSE | CTRL | ON - drag to copy off/
									// with or without holding Ctrl
		...
		return {
			...
			attach_events : function() {
				...
				$("li", this.container.get(0))
						.live(
								"click",
								function(event) { // WHEN CLICK IS ON THE
													// ARROW
				...
				$("a", this.container.get(0))
						.live(
								"click",
								function(event) { // WHEN CLICK IS ON THE TEXT
													// OR ICON
						...
						.live(
								"dblclick",
								function(event) { // WHEN DOUBLECLICK ON TEXT
													// OR ICON
									if (_this.locked) {
										event.preventDefault();
										event.stopPropagation();
			...
			scroll_check : function(x, y) {
				...
				if (cnt.height() - (y - off.top) < h_cor)
					cnt.scrollTop(st + _this.settings.ui.scroll_spd); // NEAR
																		// BOTTOM
				if (x - off.left < 20)
					cnt.scrollLeft(Math.max(
							(sl - _this.settings.ui.scroll_spd), 0)); // NEAR
																		// LEFT
				if (cnt.width() - (x - off.left) < 40)
					cnt.scrollLeft(sl + _this.settings.ui.scroll_spd); // NEAR
																		// RIGHT
			...
			open_branch : function(obj, disable_animation, callback) {
				...
				if (this.settings.data.async && obj.find("li").size() == 0) {
					...
					var _datastore = new $.tree.datastores[this.settings.data.type]();
					_datastore.load(this.callback("beforedata", [ obj, this ]),
							this, this.settings.data.opts, function(data) {
								data = _this
										.callback("ondata", [ data, _this ]);
								if (!data || data.length == 0) {
									obj.removeClass("closed").removeClass(
											"open").addClass("leaf").children(
											"ul").remove();
									if (callback)
										callback.call();
									return;
								...
								_datastore
										.parse(data, _this,
												...
												function(str) {
													...
													// else
												obj.children("ul:eq(0)")
														.replaceWith(
																$("<ul>").html(
																		str));
												obj.find("li:last-child")
														.addClass("last").end()
														.find("li:has(ul)")
														.not(".open").addClass(
																"closed");
												obj.find("li").not(".open")
														.not(".closed")
														.addClass("leaf");
			...
			moved : function(what, where, how, is_new, is_copy, rb) {
				...
				if (what.size() > 1) {
					...
					what.each(function(i) {
						if (i == 0)
							return;
						if (tmp) { // if tmp is false - the previous move was a
									// no-go
								tmp = _this.moved(this,
										tmp.children("a:eq(0)"), "after",
										false, is_copy, rb);
							}
						});
					return what;
				...
				// NO LONGER CORRECT WITH position PARAM - if(is_new && how !=
				// "inside") where = this.get_node(where).parents("li:eq(0)");
				if (is_copy)
	...
	$.extend($.tree.plugins, {
		"checkbox" : {
			...
			get_unchecked : function(t) {
				if (!t)
					t = $.tree.focused();
				return t.container.find("a:not(.checked, .undetermined)")
						.parent();
			},
			check : function(n) {
				if (!n)
				...
				if (n.children("a").hasClass("checked"))
					return true;
				var opts = $.extend(true, {}, $.tree.plugins.checkbox.defaults,
						t.settings.plugins.checkbox);
				if (opts.three_state) {
					n.find("li").andSelf().children("a").removeClass(
							"unchecked undetermined").addClass("checked");
					n.parents("li").each(
							function() {
								if ($(this).children("ul").find(
										"a:not(.checked):eq(0)").size() > 0) {
									$(this).parents("li").andSelf().children(
											"a").removeClass(
					...
					n.children("a").removeClass("unchecked")
							.addClass("checked");
				return true;
			},
			uncheck : function(n) {
				...
				if (opts.three_state) {
					n.find("li").andSelf().children("a").removeClass(
							"checked undetermined").addClass("unchecked");
					n.parents("li").each(
							function() {
								if ($(this).find("a.checked, a.undetermined")
										.size() - 1 > 0) {
									$(this).parents("li").andSelf().children(
											"a").removeClass(
											"unchecked checked").addClass(
					...
					n.children("a").removeClass("checked")
							.addClass("unchecked");
				return true;
			},
			toggle : function(n) {
...
(function($) {
						...
						"contextmenu" : {
								...
								items : {
									create : {
										...
										visible : function(NODE, TREE_OBJ) {
											if (NODE.length != 1)
												return 0;
											return TREE_OBJ.check("creatable",
													NODE);
										},
										action : function(NODE, TREE_OBJ) {
											TREE_OBJ.create(false, TREE_OBJ
													.get_node(NODE[0]));
										},
									...
									rename : {
										...
										visible : function(NODE, TREE_OBJ) {
											if (NODE.length != 1)
												return false;
											return TREE_OBJ.check("renameable",
													NODE);
										},
										action : function(NODE, TREE_OBJ) {
							...
							show : function(obj, t) {
								var opts = $.extend(true, {},
										$.tree.plugins.contextmenu.defaults,
										t.settings.plugins.contextmenu);
								obj = $(obj);
								...
								for ( var i in opts.items) {
									...
									if (typeof opts.items[i].visible == "function")
										r = opts.items[i].visible
												.call(
														null,
														$.tree.plugins.contextmenu.data.a,
														t);
									if (r == -1)
										continue;
									else
									...
									if (opts.items[i].icon)
										...
										str += "<ins>&nbsp;</ins>";
									str += "<span>" + opts.items[i].label
											+ '</span></a></li>';
									if (opts.items[i].separator_after === true)
										str += "<li class='separator'><span>&nbsp;</span></li>";
								}
								var tmp = obj.children("a:visible").offset();
								$.tree.plugins.contextmenu.object.attr(
										"class",
										"tree-context tree-"
												+ t.settings.ui.theme_name
														.toString()
												+ "-context").html(str);
								var h = $.tree.plugins.contextmenu.object
										.height();
								var w = $.tree.plugins.contextmenu.object
										.width();
								var x = tmp.left;
								var y = tmp.top
										+ parseInt(obj.children("a:visible")
												.height()) + 2;
								...
								$.tree.plugins.contextmenu.object.css( {
									"left" : (x),
									"top" : (y)
								}).fadeIn("fast");
							},
							hide : function() {
								...
								if ($.tree.plugins.contextmenu.data.r
										&& $.tree.plugins.contextmenu.data.a) {
									$.tree.plugins.contextmenu.data.a.children(
											"a, span").removeClass(
											opts.class_name);
								}
								$.tree.plugins.contextmenu.data = {
									a : false,
									r : false,
									t : false
								};
							...
							exec : function(cmd) {
								...
								var opts = $
										.extend(
												...
												$.tree.plugins.contextmenu.data.t.settings.plugins.contextmenu);
								try {
									opts.items[cmd].action
											.apply(
													null,
													[
															$.tree.plugins.contextmenu.data.a,
															$.tree.plugins.contextmenu.data.t ]);
								} catch (e) {
								}
								;
							},
							callbacks : {
								oninit : function() {
									if (!$.tree.plugins.contextmenu.css) {
										...
										$.tree.plugins.contextmenu.css = this
												.add_sheet( {
													str : css
												});
									}
								...
								onrgtclk : function(n, t, e) {
									...
									if (!n.children("a:eq(0)").hasClass(
											"clicked")) {
										$.tree.plugins.contextmenu.data.a = n;
										$.tree.plugins.contextmenu.data.r = true;
										n.children("a").addClass(
												opts.class_name);
										e.target.blur();
									} else {
										$.tree.plugins.contextmenu.data.r = false;
										$.tree.plugins.contextmenu.data.a = (t.selected_arr && t.selected_arr.length > 1) ? t.selected_arr
												: t.selected;
									}
	...
	$(function() {
		...
		$("#jstree-contextmenu a").live(
				"click",
				function(event) {
					if (!$(this).hasClass("disabled")) {
						$.tree.plugins.contextmenu.exec.apply(null, [ $(this)
								.attr("rel") ]);
						$.tree.plugins.contextmenu.hide();
					}
					event.stopPropagation();
	...
	$.extend($.tree.plugins, {
		"cookie" : {
			...
			set_cookie : function(type) {
				var opts = $.extend(true, {}, $.tree.plugins.cookie.defaults,
						this.settings.plugins.cookie);
				...
				case "selected":
					if (this.settings.rules.multiple != false
						...
						var val = this.selected ? this.selected.attr("id")
								: false;
				...
				case "open":
					...
					$.cookie(opts.prefix + 'open', str.replace(/,$/ig, ""),
							opts.options);
					break;
				}
			},
			callbacks : {
				oninit : function(t) {
					...
					if (tmp) {
						tmp = tmp.split(",");
						if (opts.keep_opened)
							this.settings.opened = $.unique($.merge(tmp,
					...
					if (tmp) {
						...
						if (opts.keep_selected)
							this.settings.selected = $.unique($.merge(tmp,
									this.settings.opened));
						else
							this.settings.selected = tmp;
					}
				...
				onchange : function() {
					$.tree.plugins.cookie.set_cookie
							.apply(this, [ "selected" ]);
				},
				onopen : function() {
					...
					$.tree.plugins,
					{
						"hotkeys" : {
							...
							defaults : {
								...
								functions : {
									"up" : function() {
										$.tree.plugins.hotkeys.get_prev
												.apply(this);
										return false;
									},
									"down" : function() {
										$.tree.plugins.hotkeys.get_next
												.apply(this);
										return false;
									},
									"left" : function() {
										$.tree.plugins.hotkeys.get_left
												.apply(this);
										return false;
									},
									"right" : function() {
										$.tree.plugins.hotkeys.get_right
												.apply(this);
										return false;
									},
									"f2" : function() {
							...
							exec : function(key) {
								...
								if (typeof t.settings.plugins.hotkeys == "undefined")
									return;
								var opts = $.extend(true, {},
										$.tree.plugins.hotkeys.defaults,
										t.settings.plugins.hotkeys);
							...
							get_next : function() {
								var opts = $.extend(true, {},
										$.tree.plugins.hotkeys.defaults,
										this.settings.plugins.hotkeys);
								var obj = this.hovered || this.selected;
								return opts.hover_mode ? this.hover_branch(this
										.next(obj)) : this.select_branch(this
										.next(obj));
							},
							get_prev : function() {
								var opts = $.extend(true, {},
								...
								var obj = this.hovered || this.selected;
								return opts.hover_mode ? this.hover_branch(this
										.prev(obj)) : this.select_branch(this
										.prev(obj));
							},
							get_left : function() {
								var opts = $.extend(true, {},
								...
								if (obj) {
									if (obj.hasClass("open"))
										...
										return opts.hover_mode ? this
												.hover_branch(this.parent(obj))
												: this.select_branch(this
														.parent(obj));
									}
								}
							},
											...
											(function(k) {
												$(document)
														.bind(
																...
																function(event) {
																	return $.tree.plugins.hotkeys
																			.exec(k);
																});
											})(i);
											$.tree.plugins.hotkeys.bound
													.push(i);
										}
									}
								}
		...
		"metadata" : {
			...
			callbacks : {
				check : function(rule, obj, value, tree) {
					var opts = $.extend(true, {},
							$.tree.plugins.metadata.defaults,
							this.settings.plugins.metadata);
					if (typeof $(obj).metadata( {
						type : "attr",
						name : opts.attribute
					})[rule] != "undefined")
			...
			callbacks : {
				oninit : function(t) {
					if (this.settings.ui.theme_name != "themeroller")
						return;
					var opts = $.extend(true, {},
							$.tree.plugins.themeroller.defaults,
							this.settings.plugins.themeroller);
					this.container.addClass("ui-widget ui-widget-content");
					$("#" + this.container.attr("id") + " li a").live(
							"mouseover", function() {
				...
				onparse : function(s, t) {
					...
					var opts = $.extend(true, {},
							$.tree.plugins.themeroller.defaults,
							this.settings.plugins.themeroller);
					return $(s).find("a").not(".ui-state-default").addClass(
							"ui-state-default").children("ins").addClass(
							"ui-icon").end().end().end();
				},
...
(function($) {
							...
							return {
										...
										for ( var i in t.settings.languages) {
											...
											str += this
													.process_inner(
															obj
																	.children("a."
																			+ t.settings.languages[i]),
															t,
															opts,
															t.settings.languages[i]);
										}
									} else {
										str += this.process_inner(obj
												.children("a"), t, opts);
									}
									str += '</content>';
									str += '</item>';
								...
								process_inner : function(obj, t, opts, lang) {
									...
									if (lang)
										str += ' lang="' + lang + '" ';
									if (opts.inner_attrib.length
											|| obj.children("ins").get(0).style.backgroundImage
													.toString().length
											|| obj.children("ins").get(0).className.length) {
										if (obj.children("ins").get(0).style.className.length) {
											str += ' icon="' + obj.children(
													"ins").get(0).style.className + '" ';
										}
										if (obj.children("ins").get(0).style.backgroundImage.length) {
											str += ' icon="' + obj.children(
													"ins").get(0).style.backgroundImage
													.replace("url(", "")
													.replace(")", "") + '" ';
										}
										if (opts.inner_attrib.length) {
								...
								parse : function(data, t, opts, callback) {
									...
									processor
											.importStylesheet($.tree.datastores.xml_flat.xsl);

									var result = $((new XMLSerializer())
											.serializeToString(processor
													.transformToDocument(data)));
									if (result.is("ul"))
										result = result.html();
									else
									...
									// if(callback) callback.call(null,(new
									// XMLSerializer()).serializeToString(processor.transformToDocument(data)).replace(/^<ul[^>]*>/i,"").replace(/<\/ul>$/i,""));
								},
								load : function(data, t, opts, callback) {
									if (opts.static) {
										callback.call(null, (new DOMParser())
												.parseFromString(opts.static,
														'text/xml'));
									} else {
										$
												.ajax( {
	...
	$.tree.datastores.xml_flat.xsl = (new DOMParser())
			.parseFromString(
...
(function($) {
						...
						"xml_nested" : function() {
							return {
								...
								parse : function(data, tree, opts, callback) {
									var processor = new XSLTProcessor();
									processor
											.importStylesheet($.tree.datastores.xml_nested.xsl);
	...
	$.tree.datastores.xml_nested.xsl = (new DOMParser())
			.parseFromString(
...
$(function() {
	var h = 0;
	$("#container .source").each(
			function() {
				var code = $(this).html().replace(/</g, '&lt;').replace(/>/g,
						'&gt;');
				var div = $('<div class="code"><pre class="brush:'
						+ ($(this).is("script") ? 'js' : 'xml') + ';">' + code
						+ '</pre></div>');
				$(this).after(div);
			});
