...
var WYSIWYG = {
	...
	_generate: function(n, settings) {
		...
		if($xx("textMode" + n)) {
			$xx("textMode" + n).style.display = 'none'; 
		}
	    ...
	    for (var idx=0; idx < document.forms.length; idx++) {
	    	...
	    	WYSIWYG_Core.addEvent(doc, "keyup", function xxx_aa(e){ 
    			var evt = (window.event) ? event : e;
    			
    			if (isCtrl==true && evt.keyCode == 86) {
    				//alert(isCtrl + " : " + evt.keyCode);
    				//WYSIWYG.updateTextArea(n); 
    				WYSIWYG.removeFormat(n); 
    			}
...
//    			if(evt.keyCode == 17) 
    	...
    	WYSIWYG_Core.addEvent(doc, "keydown", function xxx_aa2(e) 
		{ 
    		var evt = (window.event) ? event : e;
...
//    		}
	...
	updateStatusBar: function(n) {
		...
		// update the status bar 	
		var statusbar = $xx("wysiwyg_statusbar_" + n);
		if(statusbar){ 
	...
	writeDropDown: function(n, id) {
		...
		output += '<td onMouseOver="$xx(\'img_' + dropdown.id + '_' + n + '\').src=\'' + imageOn + '\';" onMouseOut="$xx(\'img_' + dropdown.id + '_' + n + '\').src=\'' + image + '\';">';
	...
	closeDropDowns: function(n, exid) {
		...
		for(var id in dropdowns) {
			...
			if(dropdown.id != exid) {
				...
				if($xx(divId)) $xx(divId).style.display = 'none';
			}
		}			
	},
	...
	openDropDown: function(n, id) {
		...
		if($xx(divId).style.display == "none") {	
			$xx(divId).style.display = "block"; 
		}
		else {
			$xx(divId).style.display = "none"; 
		}
		$xx(divId).style.position = "absolute";
	...
	viewSource: function(n) {
		...
		if($xx('HTMLMode' + n)) {
		    $xx('HTMLMode' + n).style.display = 'none'; 
		}
	    if($xx('textMode' + n)) {
		    $xx('textMode' + n).style.display = 'block';
		}
	...
	viewText: function(n) { 
		...
		if($xx('HTMLMode' + n)) {
			$xx('HTMLMode' + n).style.display = 'block';
		}
